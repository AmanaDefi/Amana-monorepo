...
describe("AmanaConnectedChainVault Tests", function () {
  ...
  async function setup() {
    ...
    return { owner, user1, user2, depositAmount1, depositAmount2, rewardAmount, ethBaseSepolia, usdt, amanaVault, gatewayZEVM, withdrawZRC20 };
  }
  ...
  describe("Cross-Chain Deposit and Withdraw Workflow", function () {
    ...
    it("should withdraw the maximum amount possible for a user", async function () {
      ...
      const confirmMessage = ethers.utils.defaultAbiCoder.encode(
        ["address", "address", "uint256", "uint256", "uint32", "bool", "uint256", "uint256", "uint256"],
        [user1Address, ethers.constants.AddressZero, depositAmount1, 0, 0, true, 0, depositAmount1, 1]
      );
      await amanaVault.onCall(
        {
          origin: ethers.utils.hexlify(ethers.utils.toUtf8Bytes("test_origin")),
      ...
      const confirmMessage2 = ethers.utils.defaultAbiCoder.encode(
        ["address", "address", "uint256", "uint256", "uint32", "bool", "uint256", "uint256", "uint256"],
        [user1Address, ZC_TEST_ETH_BASESEPOLIA_ADDRESS, maxWithdrawAmount, 0, 84532, false, depositAmount1, 0, 2]
      );
      await setTokenBalance(ZC_TEST_ETH_SEPOLIA_ADDRESS, amanaVault.address, maxWithdrawAmount);

      await expect(amanaVault.onCall(
        {
          origin: ethers.utils.hexlify(ethers.utils.toUtf8Bytes("test_origin")),
          sender: STRATEGY_ADDRESS,
          chainID: STRATEGY_CHAIN_ID,
    ...
    it("should update user shares correctly after multiple deposits and withdrawals", async function () {
      ...
      const confirmMessage1 = ethers.utils.defaultAbiCoder.encode(
        ["address", "address", "uint256", "uint256", "uint32", "bool", "uint256", "uint256", "uint256"],
        [await user1.getAddress(), ethers.constants.AddressZero, depositAmount1, 0, 0, true, 0, depositAmount1, 1]
      );
      ...
      const confirmMessage2 = ethers.utils.defaultAbiCoder.encode(
        ["address", "address", "uint256", "uint256", "uint32", "bool", "uint256", "uint256", "uint256"],
        [await user2.getAddress(), ethers.constants.AddressZero, depositAmount2, 0, 0, true, depositAmount1, totalDeposits, 2]
      );
      ...
      const confirmMessage3 = ethers.utils.defaultAbiCoder.encode(
        ["address", "address", "uint256", "uint256", "uint32", "bool", "uint256", "uint256", "uint256"],
        [await user1.getAddress(), ZC_TEST_ETH_SEPOLIA_ADDRESS, withdrawAmount1, 0, 7001, false, totalDeposits, totalDeposits.sub(withdrawAmount1), 3]
      );

      await setTokenBalance(ZC_TEST_ETH_SEPOLIA_ADDRESS, amanaVault.address, withdrawAmount1);
    ...
    it("should correctly distribute rewards proportional to user shares", async function () {
      ...
      await usdt.transfer(amanaVault.address, rewardAmount);
      await amanaVault.claimRewards(await user1.getAddress());
    ...
    it("should reject deposits exceeding max deposit limit", async function () {
      ...
      ).to.be.revertedWithCustomError(amanaVault, "ERC4626ExceededMaxDeposit");
    });
