...
describe("Vault and BaseSepAaveEthStrategy", function () {
  ...
  describe("BaseSepAaveEthStrategy Investment", function () {
    async function setup() {
      ...
      console.log(await amanaVault.getAddress());
      // Deploy BaseSepAaveEthStrategy contract and set the amanaVault address
      // const BaseSepAaveEthStrategy = await ethers.getContractFactory("BaseSepAaveEthStrategy", owner);
      // strategy = await BaseSepAaveEthStrategy.deploy("AaveV3Eth", await amanaVault.getAddress(), ETH_BASESEPOLIA_ADDRESS, BASE_SEP_AAVE_ETH_RECEIPT_TOKEN_ADDRESS);
      ...
      const ethHolder = await ethers.getImpersonatedSigner(ETH_BASESEPOLIA_HOLDER_ADDRESS);

      // Set the initial balances for user1 and user2
      const depositAmount1 = ethers.parseUnits("0.01", 18); // 1000 USDC for user1
      const depositAmount2 = ethers.parseUnits("0.05", 18); // 500 USDC for user2
      console.log(depositAmount1.toString());
      console.log(depositAmount2.toString());

      await ethBaseSepolia.connect(ethHolder).transfer(await user1.getAddress(), depositAmount1 * 2n);
      await ethBaseSepolia.connect(ethHolder).transfer(await user2.getAddress(), depositAmount2 * 2n);
      await ethBaseSepolia.connect(ethHolder).transfer(await amanaVault.getAddress(), depositAmount1 * 2n);

      const userBalance = await ethBaseSepolia.balanceOf(await user1.getAddress());
      console.log("got here", userBalance.toString());

      const usdtHolder = await ethers.getImpersonatedSigner(ZC_TEST_WETH_HOLDER_ADDRESS);
    ...
    it("should invest USDC into Aave via the strategy", async function () {
      ...
      await ethBaseSepolia.connect(user1).approve(await amanaVault.getAddress(), depositAmount1);
      const userBalance = await ethBaseSepolia.balanceOf(await user1.getAddress());
      console.log("got here", userBalance.toString());
      // Deposit USDC into the amanaVault
      expect(await amanaVault.connect(user1).deposit(depositAmount1, await user1.getAddress())).to.changeTokenBalance(ethBaseSepolia, user1, depositAmount1);
    ...
    it("should withdraw USDC from Aave via the strategy", async function () {
      ...
      expect(await amanaVault.connect(user1).withdraw(withdrawAmount, await user1.getAddress(), await user1.getAddress())).to.changeTokenBalance(ethBaseSepolia, user1, withdrawAmount);
    ...
    it("should handle deposits from two different users", async function () {
      ...
      await ethBaseSepolia.connect(user1).approve(await amanaVault.getAddress(), depositAmount1);
      await ethBaseSepolia.connect(user2).approve(await amanaVault.getAddress(), depositAmount2);

      expect(await amanaVault.connect(user1).deposit(depositAmount1, await user1.getAddress())).to.changeTokenBalance(ethBaseSepolia, user1, depositAmount1);
      expect(await amanaVault.connect(user2).deposit(depositAmount2, await user2.getAddress())).to.changeTokenBalance(ethBaseSepolia, user2, depositAmount2);
    ...
    it("should handle withdrawals from two different users", async function () {
      ...
      expect(await amanaVault.connect(user1).withdraw(withdrawAmount1, await user1.getAddress(), await user1.getAddress())).to.changeTokenBalance(ethBaseSepolia, user1, withdrawAmount1);
      expect(await amanaVault.connect(user2).withdraw(withdrawAmount2, await user2.getAddress(), await user2.getAddress())).to.changeTokenBalance(ethBaseSepolia, user2, withdrawAmount2);
    ...
    it("should pay a fee to the owner upon withdrawal", async function () {
      ...
      expect(await amanaVault.connect(user1).withdraw(withdrawAmount, await user1.getAddress(), await user1.getAddress())).to.changeTokenBalance(ethBaseSepolia, owner, feeAmount);
    });

    it("should distribute and claim rewards (time-based)", async function () {
      const { user1, depositAmount1, ethBaseSepolia, usdt, amanaVault, owner } = await loadFixture(setup);
