...
contract EVM4626EthStrategy is EVMEthStrategy {
    ...
    function _depositFundsIntoYieldSource(uint256) internal override {
        weth.deposit{value: msg.value}(); // TEST this - rather use context.amount?
        bool success = weth.approve(address(receiptToken), msg.value);
        if (!success) {
            revert ApprovalFailed();
        }
        receiptToken.deposit(msg.value, address(this));
    }
