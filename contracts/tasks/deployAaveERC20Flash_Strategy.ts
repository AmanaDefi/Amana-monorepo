...
const main = async (args: any, hre: HardhatRuntimeEnvironment) => {
  ...
  const variableDebtToken = args.variableDebtToken;
  ...
  const contract = await factory.deploy(name, vault, inputToken, receiptToken, variableDebtToken, gateway);
  ...
  if (etherscanApiKey) {
    ...
    try {
      await hre.run("verify:verify", {
        ...
        constructorArguments: [name, vault, inputToken, receiptToken, variableDebtToken, gateway],
      });
...
task("deploy-aave-erc20-flash-strategy", "Deploy a Strategy contract", main)
  ...
  .addParam("variableDebtToken", "The address of the variable debt token")
