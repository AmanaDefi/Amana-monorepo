...
const SelectNetworkModal: React.FC<SelectNetworkModalProps> = ({ isOpen, onClose, walletAddress, onSelectNetwork, disconnectWallet, selectedChain }) => {
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
            ...
            <div className="bg-white p-6 rounded-lg shadow-lg w-80 gap-4 flex flex-col">
                <div className="flex justify-between items-center gap-3">
                    <h2 className="text-lg font-bold text-gray-900 text-center">{walletAddress ? 'Manage wallet' : 'Select Network'}</h2>
                    <div className="text-right"><button className=" text-gray-500 text-right" onClick={onClose}>
                ...
                {walletAddress
                    ...
                    <div className="flex flex-col gap-3">
                        <div className="text-w-full px-4 py-2 bg-black bg-opacity-95 text-white rounded-lg text-center hover:bg-opacity-85 cursor-pointer" >{`${walletAddress.slice(0, 4)}...${walletAddress.slice(-4)}`} </div>
                        <div className="text-w-full px-4 py-2 bg-black bg-opacity-95 text-white rounded-lg text-center hover:bg-opacity-85 cursor-pointer" onClick={disconnectWallet}>Disconnect </div>
                        {selectedChain == 'evm'
                            ?
