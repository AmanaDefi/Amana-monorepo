...
export const DropdownList = ({
  ...
  return (
    <div
      ...
      className={`z-10 !w-[200px] rounded-2xl absolute -right-3 flex flex-col items-center gap-3 transition-all duration-500 ease-in-out overflow-hidden
        ${isShownList ? "bg-[#161C27] border border-[#3E3C59]" : ""}
        ${isToken ? "rounded-[16px] bg-[#1D2A41] p-4" : ""} ${isIconButton && "!right-0"}`}
      role="menu"
    >
      {isIconButton && (
