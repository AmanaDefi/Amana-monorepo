...
const ChainSwitcher: React.FC = () => {
  ...
  // Handle chain switch
  const handleChainSwitch = async (chain: typeof SUPPORTED_CHAINS[number]) => {
    if (!wallet) {
      try {
  ...
  const displayChain = currentChain
    ? SUPPORTED_CHAINS.find((c: Chain) => c.id === currentChain.id) || currentChain
    : zetaChain;

  return (
    <div className="z-50 relative bg-gradient-to-b from-[#262830] to-[#06afbc] rounded-full lg:bg-none lg:rounded-none" ref={dropdownRef}>
      <button
      ...
      >
        ...
        <span className="hidden lg:block">{displayChain?.name || "Select Chain"}</span >
        <ChevronDownIcon 
        ...
        <div className="absolute top-full left-0 mt-2 w-64 bg-gray-800 rounded-md shadow-lg">
          {SUPPORTED_CHAINS.map((chain) => (
            <button
              ...
              data-tooltip-id={`chain-${chain.id}-tooltip`}
              data-tooltip-content={`Switch to ${chain.name}`}
            >
              <img
                src={chain.icon?.url}
                alt={chain.name}
              ...
              {isLoading === chain.id ? (
                ...
                <Tooltip id={`chain-${chain.id}-tooltip`} />
            </button>
          ))}
        </div>
      )}
