...
export default function Page() {
  ...
  const totalPages = useMemo(() => {
    return Math.ceil(totalItems / searchParams.perPage);
  }, [totalItems, searchParams.perPage]);
  ...
  const handleKeyPress = useCallback(
    ...
    const normalizedCurrentAddress = useMemo(() => {
      return currentUserAccount.address.toLowerCase();
  ...
  const PaginationControls = useCallback(
    ...
    [handlePageChange, searchParams.page, totalPages, totalItems]
  );
    ...
    <div className="flex flex-col py-10 lg:py-20 w-full container gap-10 lg:gap-20">
      <div className="flex-center">
        <div className="relative">
          <h1 className="text-3xl lg:text-5xl font-fustat font-semibold leading-none lg:leading-none text-white">
            Leaderboard
          </h1>
          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[105%] translate-y-full mt-1 bg-gradient-to-tr from-themeColor to-white h-0.5 lg:h-1 rounded-xl"></div>
        </div>
      </div>
                ...
                leaderboardData?.data?.map(
                  (item: LeaderboardUserData, index: number) => {
                     const isCurrentUser =
                       ...
                       item.user_address.toLowerCase() === normalizedSearchTerm;
                    return (
                      <tr
                        key={index}
                        role="button"
