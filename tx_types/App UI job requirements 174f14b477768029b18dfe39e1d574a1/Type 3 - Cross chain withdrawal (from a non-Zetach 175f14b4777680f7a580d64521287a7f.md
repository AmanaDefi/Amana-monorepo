# Type 3 - Cross chain withdrawal (from a non-Zetachain chain to a vault with a Zetachain strategy)
...
9. Message appears saying ***Initial withdraw transaction on local chain in progress***
...
11. ~~[`FundsWithdrawn` event emitted by **strategy** on **zetachain**], which triggers:
Message changes to ***Cross chain request to vault completed***~~
12. [`ReturnFundsToUserSent` event gets emitted by `AmanaZetachainVault`], which triggers:
Message changes to ***Cross chain request to vault completed ← this has now moved here!***
New line appears saying ***Return of funds in progress***
    ...
    [`CrossChainWithdrawFailed` event emitted by WithdrawalReceiver on user’s chain], which triggers:
...
13. [`FundsReturned` event gets emitted by `WithdrawalReceiver` on chain that user withdrew from], which triggers:
Message changes to ***Return of funds completed***
    ...
    [`ReturnFundsToUserFailed` event emitted by **AmanaCrossChainVault** on **Zetachain**], which triggers:
...
15. Withdraw amount reverts to zero and button disappears
16. ***Return of funds in progress - please allow 5 - 10 minutes for funds to reflect*** message disappears as soon as user performs another action (or change Deposit/Withdraw button to **Done**, waiting for user click)

### Tasks

1. We don’t need full stops on confirmation messages
